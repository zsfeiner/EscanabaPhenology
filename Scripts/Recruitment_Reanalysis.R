...
ggplot(filter(xvar.pred, !(predWAE>100 & DOY < 95)), aes(x=DOY, y=predWAE, group=fYear)) + 
  geom_point() + facet_wrap(~fYear, scales="free") + 
...
ggplot(newPE, aes(x=Year, y=PE)) + 
  geom_point() + geom_line() +
  geom_errorbar(aes(x=Year, ymax=PE+PE.sd, ymin=PE-PE.sd)) + 
  geom_hline(yintercept=coef(r.baseline))
...
ggplot(rec.dat, aes(x=Year, y=Age0PE, color=period)) + 
  geom_point()

#Perform climwin on past data following other methods from above
pastbaseline <- lm(Age0PE ~ 1, data=filter(rec.dat, period=="past"))
...
midbaseline <- lm(Age0PE ~ 1, data=filter(rec.dat, period=="mid"))
...
presentbaseline <- lm(Age0PE ~ 1, data=filter(rec.dat, period=="present"))
